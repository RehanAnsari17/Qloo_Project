...
export const RestaurantCard: React.FC<RestaurantCardProps> = ({ 
  ...
  const [submittedFeedback, setSubmittedFeedback] = useState<{preference: 'like' | 'dislike', feedback: string} | null>(null);

  useEffect(() => {
    // Load existing feedback for this restaurant if in history view
    if (userData && sessionId) {
      loadExistingFeedback();
    }
  }, [userData, sessionId, restaurant.id]);

  const loadExistingFeedback = async () => {
    if (!userData || !userData.uid || !restaurant?.id) return;
    try {
      const feedbackList = await authService.getUserFeedback(userData.uid);
      const existingFeedback = feedbackList.find(f => f.restaurantId === restaurant.id && f.sessionId === sessionId);
      
      if (existingFeedback) {
        setUserPreference(existingFeedback.preference);
        setSubmittedFeedback({
          preference: existingFeedback.preference,
  ...
  try {
    ...
    // Store the submitted feedback for display
    setSubmittedFeedback({
      preference: pendingPreference,
    ...
    <>
      ...
      >
        ...
        <div className="p-6 space-y-4">
          ...
          {/* Action Buttons */}
          <div className={`flex justify-between items-center pt-4 border-t border-gray-100 ${isHistoryView ? 'opacity-60' : ''}`}>
              ...
              <button
                ...
                className={`flex items-center space-x-2 px-4 py-2 rounded-lg transition-all duration-200 ${
                  userPreference === 'like'
                    ...
                    : `bg-gray-100 text-gray-600 border-2 border-transparent ${!isHistoryView ? 'hover:bg-red-50 hover:text-red-600' : 'cursor-not-allowed'}`
                } ${isHistoryView ? 'cursor-not-allowed' : ''}`}
              >
              ...
              <button
                ...
                className={`flex items-center space-x-2 px-4 py-2 rounded-lg transition-all duration-200 ${
                  userPreference === 'dislike'
                    ...
                    : `bg-gray-100 text-gray-600 border-2 border-transparent ${!isHistoryView ? 'hover:bg-gray-200' : 'cursor-not-allowed'}`
                } ${isHistoryView ? 'cursor-not-allowed' : ''}`}
              >
      ...
      {submittedFeedback && (
        ...
        >
          <div className="flex items-center space-x-2 mb-2">
            {submittedFeedback.preference === 'like' ? (
              <Heart className="w-4 h-4 text-red-500 fill-current" />
            ) : (
              <ThumbsDown className="w-4 h-4 text-gray-600 fill-current" />
            )}
            <span className="text-sm font-medium text-gray-700">
              Your {submittedFeedback.preference === 'like' ? 'Like' : 'Dislike'}
            </span>
          </div>
          {submittedFeedback.feedback && (
            <div className="bg-white p-3 rounded border border-gray-100">
              <p className="text-sm text-gray-600 italic">"{submittedFeedback.feedback}"</p>
            </div>
          )}
          <p className="text-xs text-gray-500 mt-2">Thank you for your feedback! This helps us improve recommendations.</p>
        </motion.div>
      )}
